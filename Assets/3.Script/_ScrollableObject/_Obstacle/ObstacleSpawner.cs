...
public class ObstacleSpawner : MonoBehaviour
{
    ...
    private float spawnInterval = 1f; // 몇초에 한번씩 소환하는 지?
    [Header("스폰 간격 숫자가 커지면 많이 소환됨)")] public float spawnRate = 1f; // 1초에 몇개를 소환하는 지?
    ...
    // spawnInterval에 의해서 주기적으로 소환시키는 메서드
    private IEnumerator SpawnRoutine() // 시간차별 소환 코루틴 메소드
    {
        while (true)
        {
            SpwanObstacle();
            float interval = spawnInterval / spawnRate; //  1 / spawnRate
            /*
    ...
    {
        // 아까 PlayerBehaviour에 있었던 캐릭터의 이동 범위 제한 데이터를 가져와서 Obstacle 소환 범위를 제한함.
    ...
    GameObject CalculateWeight() // 각 장애물 프리팹이 가진 가중치를 이용해 장애물을 생성
    {
        ...
        foreach (var obj in obstaclePrefabs) // 모든 프리팹의 weight 값을 더해 maxWeight(전체 확률범위)를 구함
